...
//		shippAddress.setUser(user);
		...
		Address address= (Address) addressRepository.save(shippAddress);



		Map<Object, List<CartItem>> itemsBySeller = cart.getCartItems().stream()
				.collect(Collectors.groupingBy(item -> item.getProduct().getSeller().getId()));

		Set<Order> orders=new HashSet<>();

		for(Map.Entry<Object, List<CartItem>> entry:itemsBySeller.entrySet()){
			Long sellerId= (Long) entry.getKey();
